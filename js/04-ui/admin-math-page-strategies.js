...
(function (root) {
  ...
  function _strategyAnalysis(type) {
    ...
    return {
      ...
      // Surface the partial-deploy breadcrumb so the migration label
      // can run the engine on the dialed-back cfg (the actual plan)
      // instead of the full cfg (which can over-show the ratchet).
      _partialDeploy: entry._partialDeploy || null,
  ...
  function _strategySection(letter, name, analysis) {
    ...
    if (combo && cfg && typeof root.unifiedTaxComparison === 'function'
      ...
      try {
        // Migration label must reflect the DIALED-BACK plan (what the
        // card represents) — at full cfg the tranching can ratchet
        // further than the optimizer's chosen plan, mislabeling a
        // "stays at 145/45" plan as "145/45 → 200/100".
        var _migCfg = cfg;
        var _pdMig = analysis._partialDeploy;
        if (_pdMig && Number.isFinite(Number(_pdMig.deployed))) {
          var _depMig = Math.max(0, Math.round(Number(_pdMig.deployed)));
          if (_depMig !== Math.round(Number(cfg.availableCapital) || 0)) {
            _migCfg = Object.assign({}, cfg, {
              availableCapital: _depMig, investment: _depMig, investedCapital: _depMig
            });
          }
        }
        if (typeof root.rettFlavorEngineCfg === 'function') {
          try { _migCfg = root.rettFlavorEngineCfg(_migCfg); } catch (e) { /* */ }
        }
        var _cmpMig = root.unifiedTaxComparison(_migCfg, { includeTrancheBreakdown: true });
    ...
    var rows = [
      ...
      (function () {
        ...
        // closing. For deferred strategies (B/C) it triggers a Y0 taxable
        // event of F × gross-profit-ratio; for A it just shrinks available
        // capital (A recognizes all gain Y0 anyway).
        var _f = _num(cfg.forcedY0Payment);
        if (_f <= 0) return '';
        var _rc = Math.max(0, _num(cfg.acceleratedDepreciation));
        var _lt = Math.max(0, _num(cfg.salePrice) - _num(cfg.costBasis) - _rc - _num(cfg.shortTermPropertyGain));
        var _ct = Math.max(0, _num(cfg.salePrice) - _rc);
        var _gp = _ct > 0 ? _lt / _ct : 0;
        return _row('Forced Y0 payment', _fmtUSD(_f),
          letter === 'A'
            ? 'Personal-use + amount-owed carved off proceeds — only reduces available capital here (A recognizes all gain Y0).'
            : 'Personal-use + amount-owed carved off at closing → Y0 taxable event of ' +
              _fmtUSD(_f * _gp) + ' LT gain (F &times; GP-ratio ' + (_gp * 100).toFixed(1) +
              '%), pulled forward out of the deferral. Not deployed to Brooklyn.');
      })(),
